...
with open('Train_Tokenized_Samplings/train_SPTL.jsonl', 'w', encoding = 'utf-8') as f:
...
for n in range(len(News)):
    if ((' '.join(wt.Space_Preprocessed_Learning_Tokenizer_Lemmatization(News[n])[0])) not in SPTLNews):
        SPTLNews.append(' '.join(wt.Space_Preprocessed_Learning_Tokenizer_Lemmatization(News[n])[0]))
with open('Test_Tokenized_Samplings/test_SPTL.jsonl', 'w', encoding = 'utf-8') as f:
    for article in SPTLNews:
        json_line = json.dumps(article, ensure_ascii = False)
        f.write(json_line + '\n')
print('2')
...
with open('Test_Tokenized_Samplings/test_culture_SPTL.jsonl', 'w', encoding = 'utf-8') as f:
    for article in SPTLCulture:
        json_line = json.dumps(article, ensure_ascii = False)
        f.write(json_line + '\n')
with open('Test_Tokenized_Samplings/test_sport_SPTL.jsonl', 'w', encoding = 'utf-8') as f:
    for article in SPTLSport:
        json_line = json.dumps(article, ensure_ascii = False)
        f.write(json_line + '\n')
with open('Test_Tokenized_Samplings/test_travel_SPTL.jsonl', 'w', encoding = 'utf-8') as f:
